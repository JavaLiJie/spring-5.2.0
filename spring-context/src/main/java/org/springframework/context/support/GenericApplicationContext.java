 ...
 *通用的ApplicationContext实现，包含一个内部变量DefaultListableBeanFactory实例，
 * 但不采用特定的bean定义格式。实现了beandefinitionregistry接口，以便将任何bean定义读取器应用到它。
 *
 ...
 * etc).
 * 调用refresh()来初始化指定的各种applicationContext的bean。但这个方法只能调一次。具体用法看下面的 Usage example
 *
