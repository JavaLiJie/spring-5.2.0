...
class ConfigurationClassEnhancer {
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//为了处理bean间方法引用的情况，我们必须显式地检查已缓存实例的容器。
			//首先，检查所请求的bean是否为FactoryBean。如果是，创建一个子类
			//代理拦截对getObject()的调用并返回任何缓存的bean实例。
			//这确保了从@Bean方法中调用FactoryBean的语义
			//与在XML中引用FactoryBean是一样的。看到spr - 6602。
			...
			if (isCurrentlyInvokedFactoryMethod(beanMethod)) {
				...
				//工厂调用bean方法来实例化和注册bean
				//(即通过getBean()调用)->调用resolveBeanReference来实际执行
				//创建bean实例。
